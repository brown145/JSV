...
// TODO List
// - wrap all app logic in closure
// - pull var statements to top
// - "use strict"
// - need a list of defined parmas; but also should have a param list on each context
...
//   should not computed on every access.
  ...
  var JsvApp = Spine.Controller.sub({
    ...
    init: function(){Context.bind('create', this.proxy(this.addOne) );
      //Context.bind('refresh', this.proxy(this.addAll) );
      //Context.bind('refresh change', this.proxy(this.renderCount) );
      Context.fetch();
      
      this.masterList = new MasterList({ el: $('#jsv-master') });
      this.detailView = new Details({ el: $('#jsv-detail') });
  ...
  var MasterList = Spine.Controller.sub({
    elements: {
      '.items': 'items'
    },
    ...
    init: function(){
      this.list = Spine.List.sub();
      this.list.extend({
        el: this.items,
        template: rr.templates['context-master-list-item']
      });
      ...
      Context.bind('refresh', this.render);
    },
    
    render: function(){
      var items = Context.all();
  ...
  */
  var Details = Spine.Controller.sub({
    ...
    render: function(context){
      var html = Mustache.render(rr.templates['context-detail'], context, {'param-template': rr.templates['context-detail-params']});
      this.html(html);
      return this;
    }
  });
